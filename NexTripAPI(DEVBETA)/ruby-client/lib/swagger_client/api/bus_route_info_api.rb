...
module SwaggerClient
  class BusRouteInfoApi
    ...
    # @return [RouteData]
    def updateroute(route, opts = {})
      data, _status_code, _headers = updateroute_with_http_info(route, opts)
      return data
    end
    ...
    def updateroute_with_http_info(route, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: BusRouteInfoApi.updateroute ..."
      end
      # verify the required parameter 'route' is set
      if @api_client.config.client_side_validation && route.nil?
        fail ArgumentError, "Missing the required parameter 'route' when calling BusRouteInfoApi.updateroute"
      end
      # resource path
      local_var_path = "/NexTrip/update/{ROUTE}".sub('{' + 'ROUTE' + '}', route.to_s)
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: BusRouteInfoApi#updateroute\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
